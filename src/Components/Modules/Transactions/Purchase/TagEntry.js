...
const TagEntry = ({ handleCloseTagModal, selectedProduct, fetchBalance }) => {
  ...
  const handleAddTagPurStone = () => {
    ...
    localStorage.setItem(
      "tagPurStoneDetails",
      JSON.stringify(newPurchaseStoneList),
    );
    window.dispatchEvent(new Event("storage"));
    setPurStoneDetails({
      stoneName: "",
  ...
  useEffect(() => {
    const calculateTotalWeight = () => {
      const storedStoneDetails =
        JSON.parse(localStorage.getItem("tagStoneDetails")) || [];
  ...
  useEffect(() => {
    ...
    const purWastagePercentage =
      parseFloat(formData.pur_Wastage_Percentage) || 0;
  ...
  const isGoldCategory =
    formData.category &&
    ["gold", "diamond", "others"].some((metal) =>
      formData.category.toLowerCase().includes(metal),
    );
  ...
  const handleChange = async (fieldOrEvent, valueArg) => {
    ...
    setFormData((prevData) => {
      ...
      if (field === "pieace_cost" || field === "tax_percent") {
        const taxPercent =
          parseFloat(field === "tax_percent" ? value : prevData.tax_percent) ||
          0;
        const pieaceCost =
          parseFloat(field === "pieace_cost" ? value : prevData.pieace_cost) ||
          0;
    ...
    const handleKeyPress = (event) => {
      if (event.altKey && event.key.toLowerCase() === "s") {
        ...
        if (formData.Pricing === "By fixed" && pcsValue <= 0) {
          alert(
            "The product's PCS must be greater than zero to submit the form.",
          );
          return;
        }
        ...
        if (
          formData.Pricing !== "By fixed" &&
          (pcsValue <= 0 || grossWeightValue <= 0)
        ) {
          alert(
            "The product's PCS and Gross Weight must be greater than zero to submit the form.",
          );
          return;
        }
  ...
  const handleAddDesign = async () => {
    ...
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        const addedDesign = updatedDesignMasters.find(
          (design) => design.value === newDesign.design_name,
        );
  ...
  const handleSubmit = async (e) => {
    ...
    if (parseFloat(formData.Gross_Weight) > parseFloat(grossWeight)) {
      const diff = (
        parseFloat(formData.Gross_Weight) - parseFloat(grossWeight)
      ).toFixed(3);
      const confirmSubmit = window.confirm(
        `The tag gross weight exceeds by ${diff} of balance Gross Weight. Do you want to proceed?`,
      );
      if (!confirmSubmit) {
  ...
  const handleAddSubCategory = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        // Use 201 instead of 200 for created status
        ...
        const updatedSubCategories = await axios.get(
          `${baseURL}/get/subcategories`,
        );
        const filteredSubCategories = updatedSubCategories.data.filter(
          (subCategory) =>
            subCategory.category_id === selectedProduct.product_id,
        );

        const addedSubCategory = filteredSubCategories.find(
          (sub) => sub.sub_category_name === newSubCategory.name,
        );
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `${baseURL}/entry/${selectedProduct.product_id}/${selectedProduct.tag_id}`,
      );
      if (!response.ok) {
  ...
  const handleEdit = (rowData) => {
    ...
    if (rowData.Making_Charges_On === "MC %") {
      ...
      rowData.Making_Charges_On === "MC / Gram" ||
      rowData.Making_Charges_On === "MC / Piece"
    ) {
      setFormData((prevData) => ({
        ...prevData,
        MC_Per_Gram_Label: "MC/Gm",
  ...
  return (
    <div style={{ paddingTop: "0px" }}>
      <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
        ...
        <h4 style={{ margin: "0" }}>
          Gross Weight: {grossWeight !== null ? grossWeight : "0"}
        </h4>
      </div>
                      ...
                      <div className="purchase-form-left">
                        <Col className="tag-urd-form1-section">
                          ...
                          <Row className="mt-3">
                            ...
                            {(formData.Making_Charges_On === "MC / Gram" ||
                              formData.Making_Charges_On === "MC / Piece") && (
                              <Col xs={12} md={2}>
                                <InputField
                                  label="MC"
                                  name="Making_Charges"
                      ...
                      <div className="purchase-form-right">
                        <Col className="tag-urd-form2-section">
                          ...
                          <Row className="mt-3">
                            ...
                            <Col xs={12} md={3}>
                              <InputField
                                ...
                                options={[
                                  ...
                                  ...purityOptions
                                    .filter(
                                      (option) => option.name && option.purity,
                            ...
                            {/* Show Making_Charges field only when Making_Charges_On is "MC / Gram" or "MC / Piece" */}
                            {(formData.pur_Making_Charges_On === "MC / Gram" ||
                              formData.pur_Making_Charges_On ===
                                "MC / Piece") && (
                              <Col xs={12} md={3}>
                                <InputField
                ...
                >
                  <thead style={{ fontSize: "14px", fontWeight: "bold" }}>
                    <tr>
                      ...
                      <th>Wt BW</th>
                      <th>W.Wt</th>
                      <th>{formData.MC_Per_Gram_Label || "MC"}</th>{" "}
                      {/* Dynamically Updated */}
                      <th>Total Wt</th>
                      <th>Image</th>
                      <th>Barcode</th>
